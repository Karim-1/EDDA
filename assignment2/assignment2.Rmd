...
```
The Q-Q plot seems fairly normal. This combined with the residuals, which are normally distributed

**2d)** Perform the Friedman test to test whether their is an effect of interface
```{r}
...
summary(studentsaov)
```
p-value < 0.05 thus we reject H0. There is a significant difference between interfaces. It is not wrong to use a one-way ANOVA test because the skill variable is also how it is (uniformly) spread over the population of people who would use the interfaces, meaning that it represents the society. This makes it so that only testing for interface is also viable.
...
plot(stepuplm, 2)
```
The plot of the residuals against each variable not in the model did not show any linear model and thats why they are not shown.
After removal of the outliers, the qq-plot shows a linear line. ??? is dat zo?
