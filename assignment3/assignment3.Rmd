...
The total count of males and females in each PClass:
```{r}
tot = xtabs(~Sex+PClass, data=titanic); tot
```

The total count of males and females in each PClass that survived:
```{r}
tot.s = xtabs(Survived~Sex+PClass, data=titanic); tot.s
```

The percentage of males and females in each PClass that survived:
```{r}
round(tot.s/tot, 2)
```
