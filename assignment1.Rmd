...
b) We then test wether the data stems from an exponential distribution with $\lambda$ from [.01, .1] .by bootstrapping with the median as the test statistic. We use $\lambda=.03$.
...
cat("lambda for p = .99:", lambdas[max_index_lamb])
```

In the graph, we find a peak for $\lambda = .024$. This implies that the distribution of the data comes from an exponential distribution with $\lambda = .24$. 
