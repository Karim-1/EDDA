...
Next, we test whether the fraction of the bills less than 10 euro is less than 25%. We can do this by bootstrapping a 95% confidence interval with a test statistic of 10.
...
succceses = sum(corrected_tel_data < 10)
failures = sum(corrected_tel_data >= 10)
binom.test(c(successes, failures), p = .25, alternative

```

With our 95% one-sided confidence interval, we find that 28.1% of the bills are less than 10 euro's within a 95% one-sides confidence interval. Therefore, we should reject the hypothesis that the fraction of the bills less than 10 euro is less than 25%.
