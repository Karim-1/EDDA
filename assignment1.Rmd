...
First, we check the normality of the data. This is done with a QQ-plot, as well as with the Shapiro-Wilk test. 

We observe the following QQ-plot:
```{r}
birth= read.table("birthweight.txt",header=TRUE)
birthweight = birth$birthweight
qqnorm(birthweight)
```
The dots in the plot seem aligned relatively straight, implying that the data is normally distributed. To be sure of this conclusion, the normality of the data is also tested with the Shapiro-Wilk test.
...
cat('\n90% confidence interval = [', mean(birthweight) + qnorm(.05),',', mean(birthweight) + qnorm(.95),']')
```
...
c) Next, we construct a 95% bootstrap confidence interval for the population median of the sample. This is done by iteratively resampling sub-datasets from the original dataset with replacement. We do this 5000 times, in order to get robust results.
...
Next, we test whether the fraction of the bills less than 10 euro is less than 25%. We can do this by with the binomial test. The binomial test assumed that data samples are independent, which is the case in our experiment. 
...
binom.test(c(under10, over10), p = .25, alternative = "greater")

```

The binomial test gives us a p-value of .8, meaning that we should not reject our h0 hypothesis, meaning that the hypothesis that the fraction of the bills less than 10 euro is less than 25% is not rejected. 
